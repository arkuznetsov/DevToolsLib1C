...
Функция СсылкаВСтруктуру(Значение) Экспорт
	...
	КодДополненияЗначения = СтрШаблон("%1%2(ОписаниеЗначения, Значение);",
	                        КодДополненияЗначения,
	...
	Исключение
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон("Ошибка дополнения значения: ""%1""%2%3",
		                        СокрЛП(Значение),
		                        Символы.ПС,
		                        ТекстОшибки);
...
Функция СсылкаИзСтруктуры(СтруктураЗначения) Экспорт
	...
	ОписаниеВида = ОписаниеВидаОбъектов(ПравилоЗагрузки.ТипЗначения);
	...
	КодПоискаСсылки = СтрШаблон("%1%2(ОписаниеВида, МетаОбъект, СтруктураЗначения);",
	                                 КодПоискаСсылки,
...
Процедура ДвиженияДокументаИзСтруктуры(Объект,
	...
	Для Каждого ТекДвижения Из МетаОбъект.Движения Цикл
		...
		ЗаполнитьНаборЗаписейРегистраИзСтруктуры(Объект.Движения[ТекДвижения.Имя], СтруктураДвижений[ТекДвижения.Имя]);
	КонецЦикла;
...
//    Модуль               - Произвольный    - модуль, содержащий функцию дополнения,
...
//   МетаОбъект              - ОбъектМетаданных    - Описание метаданных обрабатываемого типа значений
...
Функция НайтиЗначениеПоОписанию(ОписаниеВида, МетаОбъект, ОписаниеЗначения, Знач ПоискПоНаименованию = Ложь) Экспорт
	...
	Если ОписаниеЗначения.Свойство("ИмяПредопределенныхДанных")
	   И ЗначениеЗаполнено(ОписаниеЗначения.ИмяПредопределенныхДанных) Тогда
...
//
// Возвращаемое значение:
//   Соответствие из Структура                - правила выгрузки типов
//     *_ТипЗначения_       - Структура         - Имя выгружаемого типа значения
//       **ФункцияДополнения  - Строка            - Имя функции дополнительной обработки данных при выгрузке
//       **Модуль             - Модуль            - модуль из которого будет вызвана функция дополнения
//
...
Функция ПолучитьПравилоЗагрузкиТипа(ПолноеИмяТипа)
	...
	Если Правило = Неопределено Тогда
		...
		Правило.Вставить("ТипЗначения"  , ПолноеИмяТипа);
		Правило.Вставить("ФункцияПоиска", "НайтиЗначениеПоОписанию");
		Правило.Вставить("Модуль"       , Неопределено);
	КонецЕсли;
