...
Функция ИмяБиблиотеки() Экспорт
	...
	Возврат "БиблиотекаИнструментовРазработки";
	
КонецФункции // ИмяБиблиотеки()

// Возвращает номер версии библиотеки.
//
Функция ВерсияБиблиотеки() Экспорт
	
	Расширения = РасширенияКонфигурации.Получить(Новый Структура("Имя", "БиблиотекаИнструментовРазработки"));
	
	Возврат ?(Расширения.Количество() = 0, "", Расширения[0].Версия);
	
КонецФункции // ВерсияБиблиотеки()

// Проверяет необходимость выполнения обновления конфигурации
// при изменении версии метаданных или библиотек
//
// Параметры:
//  ВерсияМетаданных    - Строка - версия, указанная в свойствах метаданных конфигурации
//  ВерсияДанных        - Строка - версия, указанная в регистре версии подсистем
//
// Возвращаемое значение:
//  Булево    - Истина - изменена версия конфигурации или дополнительной подсистемы
//                       и требуется выполнить обработчики обновления
//              Ложь - выполнение обработчиков обновления не требуется
//
Функция НеобходимоВыполнитьОбновление(Знач ВерсияМетаданных, Знач ВерсияДанных) Экспорт

	ТребуетсяОбновлениеПодсистем = Ложь;
	...
	Для Каждого ТекПодсистема Из ОписанияПодсистем.Порядок Цикл
		...
		Если НЕ (ОбновлениеИнформационнойБазыСлужебный.ВерсияИБ(Описание.Имя) = Описание.Версия) Тогда
			ТребуетсяОбновлениеПодсистем = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Возврат НЕ ПустаяСтрока(ВерсияМетаданных) И ВерсияДанных <> ВерсияМетаданных ИЛИ ТребуетсяОбновлениеПодсистем;

КонецФункции // НеобходимоВыполнитьОбновление()
...
Процедура ПриДобавленииПодсистем(МодулиПодсистем) Экспорт
	...
	МодулиПодсистем.Добавить("бир_ОбновлениеИнформационнойБазыБИР");
...
Процедура ВыполнитьОбновлениеИнформационнойБазы() Экспорт
	...
	ОбновлениеИнформационнойБазы.ВыполнитьИтерациюОбновления(ИмяБиблиотеки(),
	                                                         ВерсияБиблиотеки(),
