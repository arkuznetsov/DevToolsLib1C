...
Функция ПолучитьОписаниеОбработкиОбслуживания(АдресОбработки, ИмяОбработки = "", ПолучатьОбъект = Ложь) Экспорт
	...
	ВремОбработкаОбъект = бир_ОбработкиОбслуживания.ПолучитьОбработкуОбъект(АдресОбработки, ОписаниеОбработки.ИмяОбработки, ОписаниеОбработки.ЭтоВнешняяОбработка);
...
Процедура ОбъектПриСозданииНаСервере(Отказ
	...
	ВремОбработкаОбъект = бир_ОбработкиОбслуживания.ПолучитьОбработкуОбъект(Форма[ПутьКДаннымОбработкаАдресВХранилище]);
