...
Перем БезопасныйРежимПД;     // - Булево         - Истина - все методы "Выполнить/Вычислить" будут выполняться в безопасном режиме
...
//   ПреобразовыватьПрикладныеТипы    - Булево      - Истина - прикладные типы (Объекты, Ссылки, НаборыЗаписей и т.п.)
//                                                    будут прочитаны из сериализованого представления
...
//
Функция ЗначениеИзСтруктуры(СтруктураЗначения,
                            ПолучатьБитыеСсылки = Ложь,
                            ПреобразовыватьПрикладныеТипы = Истина) Экспорт
	...
	Если ТипЗначения = Тип("Массив") Тогда
		Возврат МассивИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки, ПреобразовыватьПрикладныеТипы);
	КонецЕсли;

	Если ТипЗначения = Тип("Структура") Тогда
		Возврат СтруктураИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки, ПреобразовыватьПрикладныеТипы);
	КонецЕсли;

	Если ТипЗначения = Тип("Соответствие") Тогда
		Возврат СоответствиеИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки, ПреобразовыватьПрикладныеТипы);
	КонецЕсли;

	Если ТипЗначения = Тип("СписокЗначений") Тогда
		Возврат СписокЗначенийИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки, ПреобразовыватьПрикладныеТипы);
	КонецЕсли;

	Если ТипЗначения = Тип("ТаблицаЗначений") Тогда
		Возврат ТаблицаЗначенийИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки, , , ПреобразовыватьПрикладныеТипы);
	КонецЕсли;

	Если ТипЗначения = Тип("ДеревоЗначений") Тогда
		Возврат ДеревоЗначенийИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки, , , ПреобразовыватьПрикладныеТипы);
	КонецЕсли;
	...
	Если НЕ ПреобразовыватьПрикладныеТипы И ЭтоПрикладнойТип Тогда
		Возврат СтруктураЗначения;
	КонецЕсли;
...
Функция СоздатьОбъектИзСтруктуры(СтруктураОбъекта,
	...
	МетаОбъект = КэшМетаданных()[СтрШаблонВнутр("%1.%2", ОписаниеВида.Имя, ОписаниеВида.ИмяТипа)];
...
Функция СсылкаИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь) Экспорт
	...
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблонВнутр("Ошибка поиска ссылки типа ""%1"" по описанию:%2%3",
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		...
		Если ПолучатьБитыеСсылки Тогда
			Результат = Коллекция[МетаОбъект.Имя].ПолучитьСсылку(БитаяСсылка);
		КонецЕсли;
	КонецЕсли;
...
//
Функция ПрочитатьОписаниеОбъектаИзJSON(Знач ТекстОписанияОбъекта, Знач ВСоответствие = Ложь, Знач СвойстваСоЗначениемДата = Неопределено) Экспорт
...
Функция ОбработчикЧтенияЗначенийJSON(Свойство, Значение, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		Если СтрокаНачинаетсяСВнутр(Значение, "/Date(") И Прав(Значение, 2) = ")/" Тогда
			Значение = ПрочитатьДатуJSON(Значение, ФорматДатыJSON.Microsoft);
			Если Значение < '00020101000000' Тогда
				Значение = '00010101000000';
			КонецЕсли;
		ИначеЕсли СтрокаНачинаетсяСВнутр(Значение, "/BinaryData(") И Прав(Значение, 2) = ")/" Тогда
			Значение = Base64Значение(Сред(Значение, 13, СтрДлина(Значение) - 14));
		ИначеЕсли СтрокаНачинаетсяСВнутр(Значение, "/ValueStorage(") И Прав(Значение, 2) = ")/" Тогда
			Значение = ЗначениеИзСтрокиВнутр(Сред(Значение, 15, СтрДлина(Значение) - 16));
		ИначеЕсли СтрокаНачинаетсяСВнутр(Значение, "/UniqueID(") И Прав(Значение, 2) = ")/" Тогда
			Значение = Новый УникальныйИдентификатор(Сред(Значение, 11, СтрДлина(Значение) - 12));
		ИначеЕсли Значение = "/NULL()/" Тогда
			Значение = NULL;
		ИначеЕсли Значение = "/Undefined()/" Тогда
			Значение = Неопределено;
		ИначеЕсли СтрокаНачинаетсяСВнутр(Значение, "/UnknownType(") И Прав(Значение, 2) = ")/" Тогда
...
Функция УстановитьБезопасныйРежимПД(НовыйРежим) Экспорт
	
	Если ТипЗнч(НовыйРежим) = Тип("Булево") Тогда
		БезопасныйРежимПД = НовыйРежим;
	Иначе
		БезопасныйРежимПД = Ложь;
	КонецЕсли;
	
КонецФункции // УстановитьБезопасныйРежимПД()
...
Процедура СвойствоОбъектаИзСтруктуры(Объект,
	...
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблонВнутр("Ошибка получения значения свойства ""%1"" объекта ""%2"":%3%4",
		                             ИмяСвойства,
		                             СокрЛП(Объект),
		                             Символы.ПС,
		                             ТекстОшибки);
...
Функция ПолучитьПравилоВыгрузкиТипа(ПолноеИмяТипа)
	...
	Правило = Правила[ВРег(ПолноеИмяТипа)];

	Если Правило = Неопределено Тогда
		Правило = Правила["ПОУМОЛЧАНИЮ"];
	КонецЕсли;
...
//
Процедура ДобавитьПравилоВыгрузкиТипа(Знач ТипИсточника, Знач ФункцияДополнения, Знач Модуль = Неопределено) Экспорт
	...
	ПравилаВыгрузкиТипов.Вставить(ТипИсточника, Новый Структура());
	ПравилаВыгрузкиТипов[ТипИсточника].Вставить("ФункцияДополнения", ФункцияДополнения);
	ПравилаВыгрузкиТипов[ТипИсточника].Вставить("Модуль"           , Модуль);
...
Функция ПолучитьПравилоЗагрузкиТипа(ПолноеИмяТипа)
	...
	Если Правило = Неопределено Тогда
		...
		Если ТипЗнч(Правило) = Тип("Структура") Тогда
			Правило.Вставить("ТипЗначения", ПолноеИмяТипа);
		КонецЕсли;
	КонецЕсли;
...
//                                                          из модуля обработки (этот модуль)
//
Процедура ДобавитьПравилоЗагрузкиТипа(Знач ТипИсточника, Знач ТипПриемника, Знач ФункцияПоиска, Знач Модуль = Неопределено) Экспорт
...
//
Функция СтруктураИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь, ПреобразовыватьПрикладныеТипы = Истина)
	...
	Для Каждого ТекЭлемент Из СтруктураЗначения Цикл
		Если ПредставленияРавны(ТекЭлемент.Ключ, "__Тип") Тогда
			...
			Продолжить;
		КонецЕсли;
		ВремСтруктура.Вставить(ТекЭлемент.Ключ,
		                       ЗначениеИзСтруктуры(ТекЭлемент.Значение, ПолучатьБитыеСсылки, ПреобразовыватьПрикладныеТипы));
	КонецЦикла;
...
//
Функция МассивИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь, ПреобразовыватьПрикладныеТипы = Истина)
	...
	Для Каждого ТекЭлемент Из Элементы Цикл
		ВремМассив.Добавить(ЗначениеИзСтруктуры(ТекЭлемент, ПолучатьБитыеСсылки, ПреобразовыватьПрикладныеТипы));
	КонецЦикла;
...
//
Функция СоответствиеИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь, ПреобразовыватьПрикладныеТипы = Истина)
	...
	Для Каждого ТекЭлемент Из Элементы Цикл
		ВремСоответствие.Вставить(ЗначениеИзСтруктуры(ТекЭлемент.Ключ, ПолучатьБитыеСсылки, ПреобразовыватьПрикладныеТипы),
		                          ЗначениеИзСтруктуры(ТекЭлемент.Значение, ПолучатьБитыеСсылки, ПреобразовыватьПрикладныеТипы));
	КонецЦикла;
...
//
Функция СписокЗначенийИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь, ПреобразовыватьПрикладныеТипы = Истина)
	...
	Для Каждого ТекЭлемент Из Элементы Цикл
		ВремСписок.Добавить(ЗначениеИзСтруктуры(ТекЭлемент.Значение, ПолучатьБитыеСсылки, ПреобразовыватьПрикладныеТипы),
...
Процедура СтрокаЗначенийИзСтруктуры(СтрокаЗначений,
                                    ...
                                    ПолучатьБитыеСсылки = Ложь,
                                    СписокСвойств = Неопределено,
                                    ПреобразовыватьПрикладныеТипы = Истина)
...
Функция ТаблицаЗначенийИзСтруктуры(СтруктураЗначения,
	...
	Для Каждого ТекСтрока Из Строки Цикл
		...
		СтрокаЗначенийИзСтруктуры(НоваяСтрока,
		                          ТекСтрока,
		                          ПолучатьБитыеСсылки,
		                          ВремТаб.Колонки,
		                          ПреобразовыватьПрикладныеТипы);
...
Процедура СтрокиДереваИзСтруктуры(СтрокиДерева,
	...
	Для Каждого ТекСтрока Из СтруктураСтрок Цикл
		...
		СтрокаЗначенийИзСтруктуры(НоваяСтрока,
		                          ...
		                          СписокСвойств,
		                          ПреобразовыватьПрикладныеТипы);
		СтрокиДереваИзСтруктуры(НоваяСтрока.Строки,
		                        ТекСтрока.Строки,
		                        ПолучатьБитыеСсылки,
		                        СписокСвойств,
		                        ПреобразовыватьПрикладныеТипы);
...
Функция СтрШаблонВнутр(Знач Текст, Замена1 = Неопределено, Замена2 = Неопределено, Замена3 = Неопределено,
	...
	Если Совместимость8_3(6) Тогда
		Если БезопасныйРежимПД() Тогда
			УстановитьБезопасныйРежим(Истина);
		КонецЕсли;
		КодЗамены = "СтрШаблон(Текст";
		Для й = 1 По 10 Цикл
			Значение = Вычислить(СтрШаблон("Замена%1", й));
			Если Значение = Неопределено ИЛИ Найти(Текст, "%" + й) = 0 Тогда
				Прервать;
			КонецЕсли;
			КодЗамены = СтрШаблон("%1, Замена%2", КодЗамены, й);
		КонецЦикла;
		КодЗамены = СтрШаблон("%1)", КодЗамены);
		Возврат Вычислить(КодЗамены);
	КонецЕсли;	
