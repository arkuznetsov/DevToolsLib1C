...
Функция ПолучитьРезультатВыполненияКодаПроверки(ИмяПоляНастройки_Код, Источник, Событие, ПараметрыПодписки, ПараметрыСобытия)
	...
	Исключение
		...
		Разделитель = "========================================================";
		ТекстОшибки = СтрШаблон("Ошибка:%1%2%1 При выполнения кода:%1%3%1%4%1%3",
		                        Символы.ПС,
								ТекстОшибки,
								Разделитель,
								СокрЛП(ИсполняемыйКод));
		ПараметрыСобытия.Вставить("ОшибкаПроверки", Истина);
		ПараметрыСобытия.Вставить("РезультатПроверки", ТекстОшибки);
	КонецПопытки;
